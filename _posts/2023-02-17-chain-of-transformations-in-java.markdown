...
It happened that I had the need to optimize a piece of our product's business logic.
This specific part of code had to manipulate a string and return in output a "sanitized" value.

Here some pseudo code about it looked like:
```java
String sanitize(final String input) {
    String a1 = doSomething(input);
    String a2 = doSomethingElse(a1);
    String a3 = doSomethingElseAndMore(a2);
    String result = prettify(a3);
...
Let's say that, in this case, the easiest path for me was to just add an additional line in this method, in the right point, that did what I need, and my job was done.
...
So I tough that it was worth invest a small percentage of the time dedicated to this in order to review and improve this method.
...
Code style a part, I love to approach to problem with a functional point of view, but this doesn't means that a "more classic Java" approach is wrong.

Even because, looking at this snippet (assuming that all the functions doesn't have side-effects), this is a pure function.
...
So, at this point it sure that I will review this part of code. Let's resume the situation: 
...
Let's focus now on the highlights: 

1. Conceptually we have a list of operations. It means that we could have 1 or N transformation to apply and the order matters;
...
Above, in point 1. and 3. it was specified that we're talking about a list of transformation. For transformation we mean an operation that takes an input of type X and returns an output of type X.

Since in this example we're talking about Strings, we can resume our transformations as `List<Function<String, String>>`

Let's define them so: 
...
If in the future will come the need to add/remove an operation, it's just the matter or modify this list.

At this point it just about understand how to let it land,
and what we would obtain is to map that list in something like: 
...
Then need here is to combine our values into one and Streams provide the [reduce](https://docs.oracle.com/en/java/javase/18/docs/api/java.base/java/util/stream/Stream.html#reduce(T,java.util.function.BinaryOperator)) method for this purpose.
...
The reduce method has this signature: `T reduce(T identity, BinaryOperator<T> accumulator)` with `T == Function<String, String>` in this example.
...
Second parameter `BinaryOperator<T> accumulator` allows to drive how the accumulator should behave by defining how an element A and his next element B should interact in order to return a single element of the same time.
Probably coding this sentence is easier than explain it. 
Since want this example wants to achieve is to: *run function B with input the output of A.* the easiest way to achieve it is: `(a, b) -> a.andThen(b)`.
Even in this case the `Function` interface has a method that comes as help.
...
The job is done, let's glue stuff together and obtain a pointer to that chain of transformation.
...
The order matters. Indeed looking in example's transformations the third output line still present a whitespace at the begin even if one of the function is a `trim`.
This happens because the chain is firstly run the trim function, and then is manipulating the string removing a word. `"Hello world" -> becomes -> " world"`.

At this point it's enough review the order of the transformations, but chain's order now is hardcoded. 
How to improve it and let it be driven by and extern factor (like a system property, DB record, configuration file)?
...
There are tons of way to do it but the easiest that, at the time, comes in my mind was to assign at each transformation an identifier (key) and let the order be driven by a list that reference to these keys.
...
Now let's define the order with a list of these keys: (let's remember that our aim is to trim the string after all the other operations)
...
Calling now the example code will produce the desiderate output (*with no whitespace at begin/end of strings*):
```java
System.out.println(chain.apply("Hi, I'm Jacopo!"));
System.out.println(chain.apply("Welcome to my"));
System.out.println(chain.apply("Hello World"));
...
Both `Streams` and `Function` allows Java developers to write better, clever and simpler code. This example aim to highlight how meaningfully they operates together.
...
Once that it's address it can be integrated with higher-level logic and being integrated with more complex systems.